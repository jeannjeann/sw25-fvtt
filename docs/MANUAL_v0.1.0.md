...
- 装備品欄
  ...
  - この欄からの新規作成、編集、削除は行えません。所持品タブの一覧から行ってください。
...
- 系統ごとの一覧からはアイテムの新規作成、編集、削除は行えません。「全魔法リスト」～行ってください。
- アイコンクリック時の動作は詳細から設定できます。デフォルトでは説明内容のチャット表示となっています。
...
- 閲覧者権限では概要のみが表示されるようになるはずです。（未実装）
- GM用メモはGMのみが閲覧できるようななるはずです。（未実装）
...
- 基本能力（通常の攻撃など）もこのアイテムを使用してください。
  ...
  - HP、MPは別枠での入力になりますが、防護点はアイテムの備考欄に「防護点:5」というように入力すると一覧に表示され、可読性が高くなります。
...
- 魔物知識判定について
  - 閲覧者権限では概要のみが表示されるようになるはずです。（未実装）
  - GM用メモはGMのみが閲覧できるようななるはずです。（未実装）
  - 現状では権限の操作によってデータ閲覧の制限が行えないので、ダミーアクターなどで対応してください。
...
- 説明タブ
  - アイテムの説明入力欄です。自由記載欄です。設定によって、アイコンクリックでチャットに表示されます。
- アイコンクリック時
  - 一覧でアイコンをクリックした時の挙動を選択できます。説明のチャット表示やダイスロールが選択できます。
- 判定使用、威力表使用
  - チェックで一覧からロールが可能になります。また、チェックすることで設定項目が表示されます。
- 判定設定
  - 判定に使用する技能、能力値ボーナス、修正を指定できます。また、「ダイス変更」にチェックを入れると2d6以外のダイスを使用できます。固定値を指定することも可能です。
- 威力表設定

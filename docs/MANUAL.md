...
#### 冒険タブ
設定された判定ロールを行うタブとなっています。主に探索パートでの使用を想定しています。
- 判定欄（左）
  ...
  - 生命抵抗力、精神抵抗力などの基本的な判定や、先制、魔物知識、判定パック、魔法行使などの技能に関連する頻用判定を登録することで利便性をあげるための一覧です。
...
### コンテンツ部
- コンテンツ部では各種アイテムの追加や編集、削除が行えます。
- 基本的に、編集モードで追加/削除、閲覧モードでは編集や参照が行えます。
- 各アイテム一覧で、技能、判定、リソース、言語を除くアイテムの名前クリックで説明が展開します。
- アイテム一覧に新規作成、編集、削除ボタンがないリストでは、説明展開時に編集、削除ボタンが表示されます。

#### 判定タブ
PCの習得している技能を設定するタブです。技能設定の他、各種技能による判定も行えます。
- 汎用技能判定欄
...
#### 装備タブ
PCの「装備」しているアイテムを表示するタブです。各種アイテムの参照やロール、装備解除が行えます。
「装備」を行うには、所持品タブからの追加/設定が必要です。
...
- 楽素、マテリアルカード、命脈点、陣気など、技能に必要なリソースが設定されている場合、該当技能の上部に表示されます。
 - リソースの±でリソースの増減、初期化などが行えます。

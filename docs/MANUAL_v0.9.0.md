...
- 練技の一覧では消費MPをクリックすることでMP消費が行われます。
...
- 注意事項
  ...
  - 種族による初期習得言語は基本的な種族のみ対応しています。インポートされない種族の場合は手動で追加する必要があります。
  - 装飾品の追加部位には対応していません。
  - 魔物の場合、魔物能力と魔法はインポートされません。多部位魔物は1つめの部位のデータのみインポートされます。
